...
class ConsultaRuc extends BaseConsultation
{
    ...
    {
        return [
            'title' => 'Consulta RUC', 'source' => 'SUNAT', 'logo' => 'sunat-logo-sin-fondo.png', 'description' => 'Superintendencia Nacional de Aduanas y de Administración Tributaria', 'field' => 'Número de RUC', 'placeholder' => 'Ingrese 11 dígitos', 'hint' => 'Ingresa los 11 dígitos del contribuyente.', 'rules' => 'required|digits:11', 'accent' => '#dc2626', 'icon' => 'search',
    ...
    protected function resolvePdfToken(): string
    {
        return $this->cacheConsultaPdf([
            ...
            'meta' => ['RUC' => $this->result['RUC'] ?? $this->busqueda],
            'filename' => 'ruc-'.($this->result['RUC'] ?? $this->busqueda),
            'sections' => [
                [
                    'heading' => 'Información del contribuyente',
                    'type' => 'fields',
                    'rows' => $this->result,
                ],
            ],
    ...
    protected function attemptReal(): ?array
    {
        ...
        if (! $resultado['success'] || empty($resultado['data'])) {
            $this->markPideFailure($resultado, 'SUNAT', 'SUNAT no devolvió resultados.');
