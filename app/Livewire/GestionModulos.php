...
final class GestionModulos extends Component
{
    ...
    public string $nuevoIconoClase = '';

    public string $nuevoIconoNombre = '';

    public function agregarIcono(): void
    {
        $data = $this->validate([
            'nuevoIconoClase' => ['required', 'string', 'max:100', 'unique:iconos,clase'],
            'nuevoIconoNombre' => ['required', 'string', 'max:100'],
        ], [], [
            'nuevoIconoClase' => 'clase del ícono',
            'nuevoIconoNombre' => 'nombre del ícono',
        ]);

        $icon = Icono::create([
            'clase' => $data['nuevoIconoClase'],
            'nombre' => $data['nuevoIconoNombre'],
            'activo' => true,
        ]);

        $this->icono = $icon->clase;
        $this->reset(['nuevoIconoClase', 'nuevoIconoNombre']);
        $this->dispatch('icon-added');
        $this->dispatch('pide-alert', message: 'Ícono agregado exitosamente.', type: 'success');
    }
