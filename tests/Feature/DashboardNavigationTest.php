...
class DashboardNavigationTest extends TestCase
{
    ...
    public function test_navigation_is_built_from_database_as_hierarchy(): void
    {
        ...
        $this->assertContains('CONADIS', array_column($consultations['children'], 'code'));

        $conadis = collect($consultations['children'])->firstWhere('code', 'CONADIS');
        $this->assertSame('conadis', $conadis['key']);
    }
    ...
    public function test_practicante_role_does_not_receive_administration_modules(): void
    {
        ...
        $this->assertNotContains('CONADIS', $codes);
    }
    ...
    public function test_home_uses_la_esperanza_identity_and_pide_branding(): void
    {
        $this->seed(PideProductionDataSeeder::class);
        $usuario = Usuario::factory()->create();
        $usuario->roles()->attach(Rol::where('codigo', 'ADMIN')->value('id'), ['fecha_asignacion' => now(), 'activo' => true]);
        ...
        Livewire::test(Dashboard::class)
            ->assertSee('Municipalidad Distrital de La Esperanza')
            ->assertSee('Sistema de Consultas PIDE')
            ->assertSeeHtml('assets/images/logo_pide_sin_texto.png')
            ->assertDontSee('Acceso institucional protegido');
    }
    ...
    public function test_level_three_tab_exposes_its_destination_to_the_loading_modal(): void
    {
        $this->seed(PideProductionDataSeeder::class);
        $usuario = Usuario::factory()->create();
        $usuario->roles()->attach(Rol::where('codigo', 'ADMIN')->value('id'), ['fecha_asignacion' => now(), 'activo' => true]);
        ...
        Livewire::test(Dashboard::class)
            ->call('selectSection', 'ruc')
            ->assertSeeHtml('data-navigation-title="SUNAT · Consulta Contribuyente"')
            ->assertSeeHtml('data-navigation-title="SUNAT · Consulta Cobranza Coactiva"')
            ->assertSeeHtml('x-text="loadingSectionTitle"');
    }
