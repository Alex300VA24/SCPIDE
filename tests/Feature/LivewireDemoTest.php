...
class LivewireDemoTest extends TestCase
{
    ...
    public function test_dni_connection_error_offers_fictitious_data_only_after_confirmation_in_production(): void
    {
        $this->app->instance('env', 'production');

        $service = \Mockery::mock(ReniecServiceInterface::class);
        $service->shouldReceive('consultarDNI')->once()->andReturn([
            'success' => false,
        ...
        Livewire::test(ConsultaDni::class)
            ...
            ->assertSet('result', [])
            ->assertSet('connectionFallbackAvailable', true)
            ->assertSet('showFictitiousModal', true)
            ->call('useFictitiousData')
            ->assertSet('usingFictitiousData', true)
            ->assertSet('showFictitiousModal', false)
            ->assertSet('result.DNI', '74251836')
            ->assertSee('Datos ficticios:');
    }

    public function test_dni_connection_error_does_not_expose_fictitious_fallback_outside_production(): void
    {
        ...
        Livewire::test(ConsultaDni::class)
            ...
            ->set('dniUsuario', '12345678')
            ->call('search')
            ->assertSet('connectionFallbackAvailable', false)
            ->assertSet('showFictitiousModal', false)
            ->call('useFictitiousData')
            ->assertSet('usingFictitiousData', false)
            ->assertSet('result', [])
            ->assertDontSee('Llenar con datos ficticios');
    }
    ...
    public function test_ayuda_filters_guides_by_role_modules(): void
    {
        ...
        Livewire::actingAs($admin)
            ...
            ->assertSee('CONADIS · Consulta por DNI')
